...
public class BrowseServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String browseType = request.getParameter("type");
        String browseTerm = request.getParameter("term");
        ...
        try {
            ...
            if (browseType.equals("title")) {
                ...
                rs = browseByTitle(browseTerm, offset, conn, statement, psize, sortmethod,
                        sortorder);

            } // browsing by title
            else {
                rs = browseByGenre(browseTerm, offset, conn, statement, psize, sortmethod,
                        sortorder);
            }
    ...
    private ResultSet browseByGenre(String browseTerm, int offset, Connection conn,
            PreparedStatement statement, int psize, String sortmethod, String sortorder)
    ...
    private ResultSet browseByTitle(String browseTerm, int offset, Connection conn,
            PreparedStatement statement, int psize, String sortmethod, String sortorder)
            throws ServletException {
        String query = "SELECT\n" + "    B.movieid as movieid,\n" + "    B.title as title,\n"
                + "    B.year as year,\n" + "    B.director as director,\n"
                + "    B.rating as rating,\n" + "    genres.name AS genrename,\n"
                + "    stars.name AS starname,\n" + "    B.starId as starId, " + "    B.price \n"
                + "FROM\n" + "    (\n" + "        SELECT\n" + "            A.movieid,\n"
                + "            A.title,\n" + "            A.year,\n" + "            A.director,\n"
                + "            A.rating,\n" + "            A.price, \n"
                ...
                + "                    movies\n"
                + "                LEFT JOIN ratings ON movies.id = ratings.movieId\n"
                + "where movies.title like ?" + "                ORDER BY\n";
        if (sortmethod.equals("title")) {
            query += " movies.title ";
        } else {
            query += " ratings.rating ";
